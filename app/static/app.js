...
$("logoutBtn").addEventListener("click", async () => { await fetch("/auth/logout", { method: "POST" }); window.location.assign("/"); });
...
$("saveDocumentBtn").addEventListener("click", async () => { if (!activeDocument || activeDocument.key !== $("documentSelect").value) return toast("Load the document before saving it"); const form = new FormData(); form.set("markdown", $("documentEditor").value); form.set("expected_version", activeDocument.version); try { const response = await fetch(`/api/documents/${activeDocument.key}`, { method: "POST", body: form }); if (!response.ok) { const body = await response.json().catch(() => ({})); throw new Error(body.detail || response.statusText); } toast("Saved to GitHub main"); await loadDashboard(); activeDocument = await loadDocument($("documentSelect").value); $("documentEditor").value = activeDocument.markdown; } catch (error) { toast(error.message); } });
